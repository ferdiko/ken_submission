...
class Consumer:
    ...
    def __init__(self, gpu_servers, log_file_prefix="log", num_queues=3):
        ...
        for server_idx in range(num_servers):
            # self.loop.create_task(self.poll_queue(server_idx, 0)) # TODO: Hacky for now, we need to poll at capacity
            for queue_idx in range(num_queues):
               self.loop.create_task(self.poll_queue(server_idx, queue_idx))
    ...
    async def poll_queue(self, server_idx, queue_idx):
        gpu_server = self.gpu_servers[server_idx]

        while True:
            # Poll queue.
            done_ids, done_preds = await gpu_server.check_queue.remote(queue_id=queue_idx)
