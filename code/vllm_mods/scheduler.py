...
class Scheduler:
    ...
    def num_queued_tokens(self):
        waiting_tokens = 0
        for seq_group in self.waiting:
            waiting_tokens += seq_group.seqs[0].get_len()
        return waiting_tokens
        

    def _del_seq_from_deque(self, queue, request_id):
        idx = 0
        while idx < len(queue):
            if queue[idx].request_id == request_id:
                del queue[idx]
                return
            idx += 1


    def free_requests(self, request_ids: List[int]):
        # print()
        # print("rids:", request_ids)
        # print("waiting:", [seq_group.request_id for seq_group in self.waiting])
        # print("running:", [seq_group.request_id for seq_group in self.running])
        # print()

        # Get seq groups to free.
        freed_seq_groups = []
        for seq_group in self.running + self.waiting:
            if int(seq_group.request_id) in request_ids:
                freed_seq_groups.append(seq_group)

        # Free seq groups.
        for seq_group in freed_seq_groups:
            if seq_group.seqs[0].seq_id in self.block_manager.block_tables:
                self.free_seq(seq_group.seqs[0])
            seq_group.seqs[0].status = SequenceStatus.FINISHED_STOPPED

        # # Delete from queue.
        for seq_group in freed_seq_groups:
            self._del_seq_from_deque(self.running, seq_group.request_id)
            self._del_seq_from_deque(self.waiting, seq_group.request_id)

        assert len(freed_seq_groups) == len(request_ids), f"freed {len(freed_seq_groups)}, r_ids {len(request_ids)}"
